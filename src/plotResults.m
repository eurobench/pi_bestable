function plotResults(filename, personal_data, testbed_data, sorted_data)
################################################################################
...
h1 = boxplots(...
       {sorted_data.base.l_heel_strike.step_length,sorted_data.base.r_heel_strike.step_length;
        ...
        'labels',{'BASE','FREE'},'colors',{[1 .4 .4],[.4 .4 1]},'Space',0.6,'Width',1,'TickSize',0.9);
...
L = legend(h1,{'left target step','right target step'});
...
for j = 1:length(fieldNames)
    ...
    boxplots(...
      {plot_pert_data.l_step.step_length(:,1),plot_pert_data.r_step.step_length(:,1);
       ...
       ,'labels',{'STEP_1','STEP_2','STEP_3','STEP_4'},'colors',{[1 .4 .4],[.4 .4 1]},'Space',0.6,'Width',1,'TickSize',0.9);
    ...
    boxplots({plot_pert_data.l_step.target_error;plot_pert_data.r_step.target_error},...
        'labels',{'L','R'},'colors',{[1 .4 .4];[.4 .4 1]},'Space',1,'Width',1,'TickSize',0.9);
    axis([0 3 0 0.2])
    grid
    box on
    ...
    hold on
    bar(1,mean(plot_pert_data.l_step.success_rate),.5,'facecolor',[1 .4 .4],'edgecolor','none')
    errorbar(1,mean(plot_pert_data.l_step.success_rate),std(plot_pert_data.l_step.success_rate),'k')
    bar(2,mean(plot_pert_data.r_step.success_rate),.5,'facecolor',[.4 .4 1],'edgecolor','none')
